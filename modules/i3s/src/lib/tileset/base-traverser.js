...
export default class Tileset3DTraverser {
  ...
  async updateAndPushChildren(tile, stack, frameState) {
    const {options} = this;
    const children = tile.children.length ? tile.children : tile._header.children;

    for (const child of children) {
      let childTile = child;
      if (!(child instanceof Tile3DHeader)) {
        const {_tileset, _basePath} = tile;
        const childHeader = await fetchTileNode(_basePath, child.href);
        childTile = new Tile3DHeader(_tileset, childHeader, tile, _basePath);
  ...
  canTraverse(tile) {
    const {options} = this;
    if (tile.children.length === 0 && tile._header.children && tile._header.children.length === 0) {
      return false;
    }
  ...
  async executeTraversal(root, baseScreenSpaceError, frameState) {
    ...
    while (stack.length > 0) {
      ...
      if (this.canTraverse(tile)) {
        refines = await this.updateAndPushChildren(tile, stack, frameState) && parentRefines;
      }
