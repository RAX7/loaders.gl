...
export default class BaseTilesetTraverser {
  ...
  constructor(options) {
    ...
    this._traversalStackMaximumLength = 0;

    this._emptyTraversalStack = new ManagedArray();
    this._emptyTraversalStackMaximumLength = 0;
  ...
  async executeTraversal(root, frameState) {
    ...
    while (stack.length > 0) {
      ...
      if (this.canTraverse(tile, frameState)) {
        refines = await this.updateAndPushChildren(tile, frameState, stack);
      }
  ...
  async updateAndPushChildren(tile, frameState, stack) {
    const {loadSiblings, skipLevelOfDetail} = this.options;

    // update child tiles (visibility and expiration)
    await this.updateChildTiles(tile, frameState);
